...
def send_mail_alerts_failure():
    ...
    subject = "ETL Pipeline [ipm.schema Daily load] status - Red"

    message = "Hi Team,"
    message+= "\n"
    message+= "\n"
    message+=" File is not available due to delay in upstream system, so tables of ipm schema[parameters_current, parameters_history] are still not loaded into snowflake."
...
def send_success_mail_alerts(context):
    ...
    subject = "ETL Pipeline [ipm.schema Daily load] status - Green"

    message = "Hi Team,"
    message += "\n"
    message += "\n"
    message += "  Tables of ipm schema[parameters_current, parameters_history] successfully loaded into Snowflake"
    message += "\n"
    message += "\n"
    message += "Thanks,"
    message += "\n"
...
DAG_ID = DAG('inv-ipm-etl-pipeline-daily-truncate-load-prod', catchup=False, default_args=DEFAULT_ARGS,
             schedule_interval='0 10 * * *')  # Scheduled at 5:50 every day
# USING CRON 0 10 * * * Australia/Sydney

# trigger
t1 = PythonOperator(
    namespace='sophia-airflow',
    task_id='trigger_ipm_etl',
    python_callable=trigger_ipm_etl,
...
t3 = KubernetesPodOperator(namespace='sophia-airflow',
    ...
    arguments=["IPM", 'PARAMETERS_CURRENT'],
    name="task-inventory-etl-parameters-current",
    is_delete_operator_pod=True,
    task_id="task-inventory-etl-parameters-current",
    image_pull_policy='Always',
    get_logs=True,
    trigger_rule='none_failed_or_skipped',
    dag=DAG_ID)

    #ipm.parameters_history
t4 = KubernetesPodOperator(namespace='sophia-airflow',
    image="847029211010.dkr.ecr.ap-southeast-2.amazonaws.com/sophia/sophia-inventory-etl",
    cmds=["python","snowflake_daily_task.py"],
    arguments=["IPM", 'PARAMETERS_HISTORY'],
    name="task-inventory-etl-parameters-history",
    is_delete_operator_pod=True,
    task_id="task-inventory-etl-parameters-history",
    image_pull_policy='Always',
    get_logs=True,
    dag=DAG_ID)
...
t1 >> t2_check_tracker_file >> [t2_poke_tracker_file, t2_file_present] >> t3 >> t4 #>> t5 >> t6
