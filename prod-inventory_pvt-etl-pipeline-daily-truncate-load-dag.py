...
s3_tracker_file = 'redshift_upload/snowflake/tracker_file/tracker_file_'
...
def send_mail_alerts_failure():
    ...
    subject = "ETL Pipeline [inventory_pvt.schema Daily load] status - Red"

    message = "Hi Team,"
    message+= "\n"
    message+= "\n"
    message+=" File is not available due to delay in upstream system, so tables of inventory_pvt schema[reporting_demand_lost_sales_cutover, reporting_demand_lost_sales_exception] are still not loaded into snowflake."
...
def send_success_mail_alerts(context):
    ...
    subject = "ETL Pipeline [inventory_pvt.schema Daily load] status - Green"

    message = "Hi Team,"
    message += "\n"
    message += "\n"
    message += "  Tables of inventory_pvt schema[reporting_demand_lost_sales_cutover, reporting_demand_lost_sales_exception] successfully loaded into Snowflake"
    message += "\n"
    message += "\n"
    message += "Thanks,"
    message += "\n"
    ...
    msg['From'] = "asokan.ramasamy@kas-services.com"
    msg['To'] = "asokan.ramasamy@kas-services.com"
    msg['Subject'] = subject
...
def trigger_inventory_pvt_etl(**kwargs):
...
DEFAULT_ARGS = {
    ...
    'email': ['asokan.ramasamy@kas-services.com'],
...
DAG_ID = DAG('inv-inventory_pvt-etl-pipeline-daily-truncate-load-prod', catchup=False, default_args=DEFAULT_ARGS,
             schedule_interval='30 12 * * *')  # Scheduled at 5:50 every day
# USING CRON 30 11 * * * Australia/Sydney (AEST)

# trigger
t1 = PythonOperator(
    namespace='sophia-airflow',
    task_id='trigger_inventory_pvt_etl',
    python_callable=trigger_inventory_pvt_etl,
...
t2_file_present = PythonOperator(
    ...
    #reporting_demand_lost_sales_cutover
t3 = KubernetesPodOperator(namespace='sophia-airflow',
    ...
    arguments=["INVENTORY_PVT", 'REPORTING_DEMAND_LOST_SALES_CUTOVER'],
    name="task-inventory-etl-reporting-demand-lost-sales-cutover",
    is_delete_operator_pod=True,
    task_id="task-inventory-etl-reporting-demand-lost-sales-cutover",
    image_pull_policy='Always',
    get_logs=True,
    trigger_rule='none_failed_or_skipped',
    dag=DAG_ID)

    #reporting_demand_lost_sales_exception
t4 = KubernetesPodOperator(namespace='sophia-airflow',
    image="847029211010.dkr.ecr.ap-southeast-2.amazonaws.com/sophia/sophia-inventory-etl",
    cmds=["python","snowflake_daily_task.py"],
    arguments=["INVENTORY_PVT", 'REPORTING_DEMAND_LOST_SALES_EXCEPTION'],
    name="task-inventory-etl-reporting-demand-lost-sales-exception",
    is_delete_operator_pod=True,
    task_id="task-inventory-etl-reporting-demand-lost-sales-exception",
    image_pull_policy='Always',
    get_logs=True,
    dag=DAG_ID)
...
t1 >> t2_check_tracker_file >> [t2_poke_tracker_file, t2_file_present] >> t3 >> t4 #>> t6 >> t7
