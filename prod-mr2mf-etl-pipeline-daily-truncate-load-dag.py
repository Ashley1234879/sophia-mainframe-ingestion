...
def send_mail_alerts_failure():
    ...
    subject = "ETL Pipeline [mr2mf.schema Daily load] status - Red"

    message = "Hi Team,"
    message+= "\n"
    message+= "\n"
    message+=" File is not available due to delay in upstream sytem, so tables of mr2mf schema[rpv296] are still not loaded into snowflake."
...
def send_success_mail_alerts(context):
    ...
    subject = "ETL Pipeline [mr2mf.schema Daily load] status - Green"

    message = "Hi Team,"
    message += "\n"
    message += "\n"
    message += "  Tables of mr2mf schema[rpv296] successfully loaded into Snowflake"
    message += "\n"
    message += "\n"
    message += "Thanks,"
    message += "\n"
...
DAG_ID = DAG('inv-mr2mf-etl-pipeline-daily-truncate-load-prod', catchup=False, default_args=DEFAULT_ARGS,
             schedule_interval='15 8 * * *')
# USING CRON 15 7 * * * Australia/Sydney

# trigger
t1 = PythonOperator(
    namespace='sophia-airflow',
    task_id='trigger_mr2mf_etl',
    python_callable=trigger_mr2mf_etl,
...
t3 = KubernetesPodOperator(namespace='sophia-airflow',
    ...
    arguments=["MR2MF", 'RPV296'],
    name="task-inventory-etl-rpv296",
    is_delete_operator_pod=True,
    task_id="task-inventory-etl-rpv296",
...
t5 = PythonOperator(
    ...
    dag=DAG_ID)
'''
t1 >> t2_check_tracker_file >> [t2_poke_tracker_file, t2_file_present] >> t3 #>> t4 >> t5
