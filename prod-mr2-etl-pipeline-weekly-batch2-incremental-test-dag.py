...
s3_tracker_file = 'redshift_upload/snowflakedfincriment/tracker_file/tracker_file_df_forecast_'
...
def send_mail_alerts_failure():
    ...
    message+= "\n"
    message+= "\n"
    message+=" File is not available due to delay in upstream sytem, so tables of mr2 schema[df_forecast] are still not loaded into snowflake."
...
def send_success_mail_alerts(context):
    ...
    message += "\n"
    message += "\n"
    message += "  Tables of mr2 schema[df_forecast] successfully loaded into Snowflake"
    message += "\n"
    message += "\n"
    message += "Thanks,"
    message += "\n"
...
DAG_ID = DAG('inv-mr2-etl-pipeline-weekly-batch2-incremental-load-prod', catchup=False, default_args=DEFAULT_ARGS,
             schedule_interval='00 17 * * MON')  # Scheduled at 5:50 every day
# USING CRON 00 17 * * MON Australia/Sydney
...
t3 = KubernetesPodOperator(namespace='sophia-airflow',
                           ...
                           arguments=["MR2", 'DF_FORECAST'],
                           name="task-inventory-etl-df-forecast",
                           is_delete_operator_pod=True,
                           task_id="task-inventory-etl-df-forecast",
