...
def send_mail_alerts_failure():
    ...
    message+= "\n"
    message+= "\n"
    message+=" File is not available due to delay in upstream sytem, so tables of mr2 schema[product_country_profile] are still not loaded into snowflake."
...
def send_success_mail_alerts(context):
    ...
    message += "\n"
    message += "\n"
    message += "  Tables of mr2 schema[product_country_profile] successfully loaded into Snowflake"
    message += "\n"
    message += "\n"
    message += "Thanks,"
    message += "\n"
...
DAG_ID = DAG('inv-mr2-etl-pipeline-daily-truncate-load-prod', catchup=False, default_args=DEFAULT_ARGS,
...
t3 = KubernetesPodOperator(namespace='sophia-airflow',
    ...
    arguments=["MR2", 'PRODUCT_COUNTRY_PROFILE'],
    name="task-inventory-etl-product-country-profile",
    is_delete_operator_pod=True,
    task_id="task-inventory-etl-product-country-profile",
