...
def send_mail_alerts_failure():
    ...
    subject = "ETL Pipeline [mainframe.schema Daily load] status - Red"

    message = "Hi Team,"
    message+= "\n"
    message+= "\n"
    message+=" File is not available due to delay in upstream system, so tables of mainframe schema[VJR100, VJR232] are still not loaded into snowflake."
...
def send_success_mail_alerts(context):
    ...
    subject = "ETL Pipeline [mainframe.schema Daily load] status - Green"

    message = "Hi Team,"
    message += "\n"
    message += "\n"
    message += "  Tables of mainframe schema[VJR100, VJR232] successfully loaded into Snowflake"
    message += "\n"
    message += "\n"
    message += "Thanks,"
    message += "\n"
...
def trigger_mainframe_etl(**kwargs):
...
DAG_ID = DAG('inv-mainframe-etl-pipeline-daily-truncate-load-prod', catchup=False, default_args=DEFAULT_ARGS,
             schedule_interval='30 7 * * *')  # Scheduled at 5:50 every day
# USING CRON 0 9 * * * Australia/Sydney

# trigger
t1 = PythonOperator(
    namespace='sophia-airflow',
    task_id='trigger_mainframe_etl',
    python_callable=trigger_mainframe_etl,
...
t3 = KubernetesPodOperator(namespace='sophia-airflow',
    ...
    arguments=["MAINFRAME", 'VJR100'],
    name="task-inventory-etl-vjr100",
    is_delete_operator_pod=True,
    task_id="task-inventory-etl-vjr100",
...
t4 = KubernetesPodOperator(namespace='sophia-airflow',
    image="847029211010.dkr.ecr.ap-southeast-2.amazonaws.com/sophia/sophia-inventory-etl",
    cmds=["python","snowflake_daily_task.py"],
    arguments=["MAINFRAME", 'VJR232'],
    name="task-inventory-etl-vjr232",
    is_delete_operator_pod=True,
    task_id="task-inventory-etl-vjr232",
    image_pull_policy='Always',
    get_logs=True,
    dag=DAG_ID)
